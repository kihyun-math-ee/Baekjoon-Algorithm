# Baekjoon Algorithm Archive 📚

This repository serves as an archive for my algorithm problem-solving codes from **Baekjoon Online Judge (BOJ)** and tracks my learning progress.

## 🧑‍💻 Profile
- **Name**: Kihyun (Electroma∫ter)
- **Major**: Mathematics & Electrical Engineering
- **Interests**: Data Structures, Algorithms, AI/ML
- **GitHub**: [kihyun-math-ee](https://github.com/kihyun-math-ee)
- **Contact**: [kihyun.math.ee@gmail.com](mailto:kihyun.math.ee@gmail.com)

## 📂 Languages
- **Python** (Main)
- C++

## 📝 Commit Rules
Commit messages must include the problem number and title.
- `Add`: Add new solution (e.g., `Add: BOJ 1000 A+B`)
- `Fix`: Bug fix or refactoring
- `Docs`: Update README or comments

---
*"Consistency is the key to mastery."*

## 📊 Repository Summary

- **Archive**: BOJ problem-solving code and chronological study notes.
- **Archived BOJ solutions**: 328 distinct problem-numbered Python files as of 2026-09-24. This counts repository files, not verified accepted submissions.
- **Organization**: Each numbered file is placed under its primary topic or event folder. The filename is the BOJ problem number.
- **Early practice**: Three non-BOJ Python exercises are kept in [`Practice`](Practice/README.md) and excluded from the count.

## 📈 GitHub Activity

![GitHub contribution streak](https://streak-stats.demolab.com/?user=kihyun-math-ee&theme=default&hide_border=true&locale=en&timezone=Asia%2FSeoul)

## 📂 Solution Categories

| Category | BOJ Solutions |
| :--- | ---: |
| [Arrays](Arrays/README.md) | 12 |
| [Backtracking](Backtracking/README.md) | 16 |
| [Binary Search](Binary%20Search/README.md) | 6 |
| [BruteForce](BruteForce/README.md) | 13 |
| [Combinatorics](Combinatorics/README.md) | 10 |
| [Conditionals](Conditionals/README.md) | 7 |
| [Data Structure](Data%20Structure/README.md) | 26 |
| [Dijkstra](Dijkstra/README.md) | 10 |
| [Disjoint Set](Disjoint%20Set/README.md) | 4 |
| [Dynamic Programming](Dynamic%20Programming/README.md) | 12 |
| [Events](Events/README.md) | 1 |
| [Floyd Warshall](Floyd%20Warshall/README.md) | 5 |
| [Geometry](Geometry/README.md) | 12 |
| [Graph Traversal](Graph%20Traversal/README.md) | 10 |
| [Greedy](Greedy/README.md) | 13 |
| [Implementation](Implementation/README.md) | 14 |
| [Loops](Loops/README.md) | 13 |
| [Math](Math/README.md) | 17 |
| [Matrix](Matrix/README.md) | 4 |
| [MST](MST/README.md) | 7 |
| [NumberTheory](NumberTheory/README.md) | 20 |
| [Prefix Sum](Prefix%20Sum/README.md) | 5 |
| [Recursion](Recursion/README.md) | 9 |
| [Set_Map](Set_Map/README.md) | 18 |
| [Sorting](Sorting/README.md) | 19 |
| [Strings](Strings/README.md) | 22 |
| [TimeComplexity](TimeComplexity/README.md) | 7 |
| [Topological Sort](Topological%20Sort/README.md) | 4 |
| [Two Pointers](Two%20Pointers/README.md) | 12 |

## 📅 Study Log

The [original chronological Study Log](STUDY_LOG.md) is preserved in full. Each category README follows the original four-column format and lists one BOJ problem per row.
